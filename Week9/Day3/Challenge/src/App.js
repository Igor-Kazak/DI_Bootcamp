...
import { setSearchField, clearText } from './actions';
...
class App extends React.Component {
  ...
  goHome = () => {
    this.setState({ posts: [], postsState: false, searchText:'' });
    this.props.clearText();
  }

  render() {
    const { rb, posts, postsState } = this.state;
    const { textToSearch, myButton } = this.props;
    ...
    if (posts.length === 0) {
      return (
        <React.Fragment>
          <SearchBox search={this.mySearch} btn={myButton} />
          <div className='tc'>
            {
              filtered.map((item, i) => {
...
const mapDispatchToProps = (dispatch) => {
  return {
    myButton: (searchText) => dispatch(setSearchField(searchText)),
